...
func (h *BITGOLDTransactionHandler) PublicKeyToAddress(pubKeyHex string) (address string, msg string, err error){
...
func (h *BITGOLDTransactionHandler) BuildUnsignedTransaction(fromAddress, fromPublicKey, toAddress string, amount *big.Int, args []interface{}) (transaction interface{}, digests []string, err error) {
...
func (h *BITGOLDTransactionHandler) SubmitTransaction(signedTransaction interface{}) (ret string, err error) {
	c, _ := rpcutils.NewClient(config.BITGOLD_SERVER_HOST,config.BITGOLD_SERVER_PORT,config.BITGOLD_USER,config.BITGOLD_PASSWD,config.BITGOLD_USESSL)
